...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($codigo) {
        $query = "UPDATE servico_tipo SET {$attr} WHERE codigo = '{$codigo}'";
    } else {
        $query = "INSERT INTO servico_tipo SET {$attr}";
    }

    if (mysql_query($query)) {
        $codigo = $codigo ?: mysql_insert_id();

        sis_logs($codigo, $query, 'servico_tipo', 'tipo de serviço');
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb shadow bg-gray-custom">
        ...
        <li class="breadcrumb-item" aria-current="page">
            <a href="#" url="<?= $servicoTipo; ?>/index.php">Tipo de Serviço</a>
        </li>
...
<div class="card shadow mb-4">
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold text-primary">
            <?= $codigo ? 'Alterar' : 'Cadastrar'; ?> Tipo de Serviço
        </h6>
    </div>
    <div class="card-body">
